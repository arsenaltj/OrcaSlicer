...
#include "SidebarMixedFilamentCreation.ipp"
#include "PlaterTextureUnits.ipp"

void Sidebar::add_mixed_filament(const std::string& target_color)
{
    auto* plater = dynamic_cast<Plater*>(GetParent());
    if (!plater) return;
    ...
    MixedFilamentDialog dlg(this, color_strs, names, types, target_color);
    if (dlg.ShowModal() == wxID_OK) {
        auto result = dlg.get_result();
        create_mixed_filament_from_result(this, result, color_strs, true);
    }
}
...
struct Plater::priv
{
    ...
    // GUI elements
    AuiMgr m_aui_mgr;
    std::unique_ptr<SmartSlicingFeatureHost> smart_slicing_host;
    ...
    bool can_begin_project_config_change() const;
...
void Plater::priv::reset_window_layout()
{
    ...
    if (smart_slicing_host != nullptr)
        smart_slicing_host->show(false);
    update_sidebar(true);
}
       ...
       // Don't make a copy on Posix. Slash is a path separator, back slashes are not accepted as a substitute.
        ...
        if (load_model) {
            ...
            if ((!is_project_file) && (!load_old_project)) {
                prepare_texture_import_units(model, texture_options, imperial_units, q, filename);
                // if (!is_project_file) {
        ...
        if (one_by_one) {
            ...
            if (model.texture_mesh && has_importable_texture(*model.texture_mesh)) {
                ...
                if (texture_import_result.matched_colors || !texture_import_result.painted.face_colors.empty()) {
...
void Plater::priv::on_process_completed(SlicingProcessCompletedEvent &evt)
{
    ...
    if (is_finished && !ai_internal_restart && smart_slicing_host != nullptr) {
        const bool success = evt.success() && !has_error && !evt.cancelled();
        smart_slicing_host->notify_slice_completed(
