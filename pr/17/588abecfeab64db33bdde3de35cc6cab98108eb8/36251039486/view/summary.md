...
版本：`005d1ce9794b` → `588abecfeab6`；模式：PR 提交。

直接修改 **41** 个文件，潜在关联 **7** 个文件；其中 **27** 个改动文件未做代码关系解析。

黄色＝直接修改，蓝色＝潜在关联；箭头由使用方指向依赖方。

```mermaid
flowchart LR
  desktop["桌面与运行时 · 修改 1 / 关联 0"]
  generation["模型生成 · 修改 11 / 关联 5"]
  slicing["智能切片 · 修改 0 / 关联 0"]
  contracts["共享契约 · 修改 0 / 关联 0"]
  adapter["Orca 适配 · 修改 0 / 关联 0"]
  core["Orca 核心 · 修改 0 / 关联 0"]
  tests["测试 · 修改 6 / 关联 2"]
  engineering["开发与协作 · 修改 19 / 关联 0"]
  assets["资源与依赖 · 修改 0 / 关联 0"]
  unknown["待归属 · 修改 4 / 关联 0"]
