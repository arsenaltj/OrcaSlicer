...
6. Reuse Orca's `ColorDecomposeRecipe` and mixed-filament configuration through `OrcaWorkspaceAdapter`; do not duplicate the solver in Python.
7. Migrate additively: derive legacy palette vectors from typed capabilities until existing callers and payloads have moved.
...
- Import automation needs a deterministic adapter around Orca's existing GUI-oriented mixing code.
