...
def download_task_artifact(
    ...
    try:
        with urllib.request.build_opener(_SafeArtifactRedirects()).open(request, timeout=_REQUEST_TIMEOUT) as response:
            content_length = response.headers.get("Content-Length")
            if content_length:
                try:
        ...
        try:
            ...
            pass
        raise TripoError("The artifact could not be downloaded.") from None
