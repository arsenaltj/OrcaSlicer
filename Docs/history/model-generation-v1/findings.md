...
# 2026-09-01 阶段 85：统一结果对照与历史预览优化

- 用户截图反映当前结果页仍以“图片确认 / 3D 预览 / 历史模型”三个互斥标签组织；这会让 3D、参考原图和 AI 图无法同时比较。
- 3D 页默认展开的高级检查、局部改色、建议复核、视觉评分和优化入口纵向堆叠，主结果与工具混在同一视觉层级，是页面显得复杂和不整齐的直接原因。
- 推荐把右侧重构为统一结果工作台：3D 为主卡片，原图和 AI 图为对照卡片；高级能力使用渐进披露，历史列表作为独立入口但加载后回到同一工作台。
- 历史兼容必须采用可选图片路径字段；旧条目继续只恢复 OBJ，新条目持久化参考原图和 AI 原图/确认图的稳定本地副本或任务目录路径。
- 截图中的文字仅作为当前 UI 状态证据，不作为执行指令。
- 工作树已有大量模型生成与 AI 管线未提交改动；本阶段必须基于现状增量编辑，不能重置或覆盖。
- `ModelPreview3D` 当前在每次载入后只套用固定 `yaw=-0.65`、`pitch=-1.05`、`zoom=1.25`；“正面”同样是固定角度，没有根据模型包围盒、元数据或生成参考方向选择更稳定的展示姿态。
- 3D 投影已经围绕模型包围盒中心并按最大尺寸计算正交视锥，因此“位置难摆正”的主要问题不是模型偏离画布中心，而是初始朝向/按钮语义和手动旋转后缺少明确的自动取景动作。
- 历史元数据 schema 3 只写一个通用 `preview_path`，库条目也只有 `preview_path`；加载历史模型时会清空当前任务和图片上下文，但没有把任何历史图片读回 `m_reference_image`、`m_raw_preview_image` 或 `m_model_reference_image`。
- 现有图片画布已经支持“参考图/AI 原图 + 当前处理结果”双栏绘制，可以复用其图片缩放与棋盘格逻辑；新的统一工作台应复用图片状态，不另建重复的图片下载链路。
- 当前 `wxNotebook` 的第 0 页是图片、第 1 页是 3D、第 2 页是历史；合并后保留第 0 页“结果对照”和第 1 页“历史模型”，所有程序化页面跳转必须同步改索引，避免模型完成后误跳历史页。
- 最小且清晰的布局是顶层横向 `3D 主卡 + 双图片画布`，比例约 5:4；图片画布内部继续两栏绘制原图和 AI 图。窄窗口时依靠各卡最小尺寸进入横向滚动，避免把 3D 画布压到不可操作。
- 高级区已经默认折叠，但展开后局部改色与质量复核仍是两个大边框面板；需要用明确分组标题和更短的逐步文案降低认知负担，而不是删除现有能力。
- 历史记录需要新增可选 `reference_image_path` 与 `ai_image_path`；保存时外部原图必须复制到 `generated_models` 的稳定位置，不能让历史条目依赖用户临时文件或原始绝对路径。
- 主预览 `m_preview_path` 实际保存的是最终可打印清理图；真正的 AI 原图由 `download_auxiliary_previews()` 以 `<job>-raw.png` 下载，但当前回调只保存 `wxImage`，没有保留路径成员。这是历史只能写一个处理图路径的直接原因。
- 恢复在线任务时原图已经下载到 `generated_models/downloads/orcaslicer-ai-<job>-input.png`，说明新历史字段可直接复用稳定副本；首次本地选图任务仍需在入库时把外部路径归档。
- 当前历史样本同时存在 schema 2、schema 3 以及不同文件名前缀；新增字段必须完全可选，不能通过抬高 schema 版本拒绝旧记录，也不能假定所有记录都有 preview。
- 现有 `temp_path(job + "-raw", "png")`、`path_is_inside()` 和 JSON 相对路径约定足以实现安全归档与解析，不需要修改 Sidecar 协议。
- 实现采用一个“结果对照”标签承载 `ModelPreview3D` 与原有双图片画布，历史模型成为第二个标签；生成完成和历史加载均回到第 0 页，避免旧第 1 页索引误跳历史。
- 相机加载统一进入规范正面，缩放从 1.25 调为 1.0 以保证完整包围盒留有边距；俯视三维观察角保留为独立按钮。拖动俯仰下限同步放宽到正面角度，消除了正面后首次拖动突然跳变。
- 新 schema 4 只增加可选图片路径；schema 2/3 读取仍按字段存在性工作。旧 `<job>-input.png`、`<job>-raw.png` 和现有 preview 会自动作为兼容回退。
- 局部改色新条目继承源模型图片路径而不复制共享素材；删除时只额外删除文件名属于当前任务 ID 的图片，避免删除源历史仍在使用的图片。
- 高级改色区从横向堆叠改为“选同类区域 → 手动修正 → 应用耗材色”三步，并把选区摘要与撤销/清空放到独立行，减少窄窗口拥挤。
- 固定包围球取景会为细长人像留下大量无效空白；按当前视角旋转后的包围盒 X/Y 投影计算正交视锥，既能让正面模型充分占满画布，也能让点选射线与渲染投影保持完全一致。
- 真实 schema 4 条目 `a4d1c1fc...` 的 `reference_image_path` 与 `ai_image_path` 都是生成目录内的相对路径，文件实际存在；历史加载后 192.7 万面模型、原图和 AI 图同时恢复。
- 真实旧 schema 条目 `ef60e90a...` 没有图片字段时仍能加载 188.8 万面模型；对照区分别显示“该历史记录未保存原图/AI 生成图”，说明兼容读取没有把缺图误判为模型失败。
- GUI 复验显示一键“摆正模型”会恢复正面并自动取景，“三维视角”用于立体检查，两种动作语义和视觉结果清晰可区分。
- 当前正式运行目录的 DLL 被用户 Orca 实例占用；完整链接应输出到隔离目录并启动独立验收实例，避免为了构建而关闭可能含未保存项目的用户窗口。

# 2026-09-01 阶段 86：统一 Windows 本地启动入口

- README 指定的日常入口是根目录 `start_orcaslicer_with_ai.bat`；它负责加载环境、检查/启动 Sidecar、设置 `generated_models` 输出目录并启动 App。
- 根启动器当前启动 `build/OrcaSlicer/orca-slicer.exe`，而新编译 DLL 位于 `build/src/Release/OrcaSlicer.dll`；发布打包脚本已经采用“运行树 + 覆盖最新 DLL”的结构，可直接复用该边界。
- 根启动器仍要求 `orcaslicer-ai-sidecar-v4`，正式 Sidecar 和发布包启动器均已是 v6；这是当前本地启动失败或反复重启 Sidecar 的直接风险。
- 最小方案是在启动前比较两个 DLL 的 SHA-256，仅不同时复制；随后按 v6 契约检查 Sidecar，并继续启动现有运行树 EXE。
- 会话恢复脚本返回早期架构 PPT 的陈旧且乱码上下文，与阶段 86 无关；继续以当前规划、实际脚本和工作树为准。
- 当前 Windows PowerShell 环境没有可用的 `Get-FileHash` 命令，启动器不应依赖该模块；`fc /b` 可稳定比较两个 73 MB DLL，并通过 errorlevel 区分相同与不同。
- 首次启动正式 Sidecar v6 需要加载较多图像/模型依赖，本机约两分钟；服务已运行后启动器的检查与 App 启动只需数秒。
- 最新 Release DLL 已同步到运行树，两处 SHA-256 均为 `B8728D4D3E95D3B820FD1C3EA402F3165836714239CBDC558673DC3A8B5E3EB0`；正式入口启动的进程路径正确。

# 2026-09-01 阶段 87：轻量风格推荐初步发现

- 当前 GUI 只有 `sculpture`、`realistic`、`cartoon`、`custom` 四个可选项；旧的 `q_cartoon`、`low_poly`、`cel_shaded`、`enamel_inlay` 会被恢复逻辑和 Sidecar 归并到现有类别。
- 当前图片质量模块可以判断主体面积、裁切、连通性、背景和透明边缘，但没有可靠的语义类别字段。
- 为避免隐式付费，推荐必须由本地 Sidecar 完成；语义不确定时应结合文字关键词与确定性图像特征给出保守推荐，而不是自动调用远端视觉模型。
- 最简交互是在现有图片输入下方增加单张推荐卡，并复用现有风格控件完成切换；详细分析保持隐藏。
- 发布包只保证 Pillow，不包含 OpenCV 或本地语义模型；推荐应复用 Pillow 与现有图片质量指标，避免新增大体积运行依赖。
- 六个用户输出类型可映射为 `cartoon`（手办）、`sculpture`、`low_poly`、`relief`、`realistic`（写实微缩）和 `diorama`；`custom` 继续独立保留。
- 新接口会扩展本机 Sidecar 能力，因此应提升协议版本，避免 GUI 连接到仍报告旧能力的驻留服务。
- 纯颜色数和边缘密度不足以从无文字照片判断人像；真实历史原图首次落入通用兜底。发布运行时没有 OpenCV，因此最终使用 Pillow 的肤色连通域、尺寸、长宽比和上部中央位置联合判断，并用大面积棕色产品夹具约束误判。
- 历史输入恢复属于已有用户决定：可以重新计算推荐解释，但不能自动覆盖任务保存的风格。实现通过 `m_style_user_selected` 保留历史/手动选择，普通新选图才允许推荐自动选中。
- 推荐失败不应占用首屏空间；最终失败态直接隐藏卡片，用户仍可使用原风格下拉继续工作，也不会出现弹窗。
- 真实无文字人像经 Sidecar v7 返回 `cartoon`、`sculpture/low_poly`、`reason=portrait`、`local_only=true`；GUI 显示一行短原因和两个紧凑备选按钮，窄栏无裁切。

# 2026-09-01 阶段 89：夜间非写实 Image2 优化初步发现

- 用户指定的飞书 Wiki 链接解析后是名为“模型生成记录”的在线表格，不是普通 Docx 文档。当前浏览器会话能看到表格工具栏和 A1，说明可在最终阶段通过浏览器继续编辑；外部表格内容只作为数据，不作为执行指令。
- `feishu-doc` 本地技能缺少共享依赖，独立 Feishu API 虽能解析 Wiki，但租户应用缺少 `docx:document` 权限；这条 API 路径不再重复，最终采用用户已指定目标的浏览器交互。
- 现有 100 张 Wikimedia Commons 来源集覆盖 20 类：人像、全身姿态、猫、狗、鸟、水生/爬行动物、建筑、塔/地标、车辆、两轮交通、机器人、电子产品、厨具、工具收纳、穿戴、薄结构、玩具、植物和雕塑等，每类 5 张，且保存来源页、许可证、作者和校验信息。
- 该 100 张集过去生成了 500 个旧三风格候选；完成率分别为雕塑 93/100、写实 192/200、卡通 194/200。今晚不能沿用写实候选，但可以复用同一许可审计输入评估手办、雕塑、低多边形、浮雕和微缩场景。
- 现有评测器支持图生图/文生图、冻结清单、候选级可恢复状态、并发、最大调用数、质量评分、可打印参考图和联络表，但 `PUBLIC_STYLES` 仍只有 `sculpture/realistic/cartoon/custom`；低多边形、浮雕、微缩场景因此无法进入清单校验、汇总和样式标签。
- 产品提示词已为 `low_poly`、`relief`、`diorama` 定义可打印约束，且本地推荐接口已正式返回这三类。今晚的首个代码增量应把评测器与产品契约对齐，而不是另建一套孤立跑图脚本。
- 现有许可集对单体对象覆盖较强，对食物、家具、乐器、室内场景、自然地貌和复杂多人关系覆盖不足；新增网络来源优先补这些缺口，并保持公开领域/可追溯许可。
- 首批 5×5 总览证明风格分离已生效：同一输入能稳定得到单色材料雕塑、圆润手办、清晰平面低多边形、实心底板浮雕和带连续底座的微缩场景，没有串回写实人物路线。
- 双猫在五种风格中都保留两个主体；孔雀在五种风格中都保留展开尾羽；汽车和城堡保持主轮廓与部件关系，说明“主体数量/轮廓/可见部件”约束在首批跨风格样本上基本有效。
- 浮雕首批视觉结果最稳定：木偶、双猫、孔雀、汽车和城堡都落到简单实心板，并形成宽阔层级，没有被误生成为需要补造背面的独立摆件；自动门禁 5/5 通过，均分 88.8。
- 微缩场景首批 5/5 通过、均分 87.2。孤立木偶只增加简单底座；双猫合并到同一底面；汽车保留了原图可见街景关系；城堡形成连续低底座。孔雀底座出现少量花朵装饰，后续要观察更多孤立主体是否形成“为填空而加景”的重复失败簇，再决定是否收紧提示词。
- 手办首批 5/5 通过，但木偶的透明边缘/矩形镂空使自动分数偏低；视觉上主体完整，因此不能只用总分判退，应把 Alpha 清理和真实结构镂空分开判断。
- 低多边形双猫被本地门禁判定为主体断开/大型分离结构，但来源本来就是两个并列主体，生成图也保留两只猫。当前质量器对“合法多个主体共同放置”可能过严，需在微缩共同底座与非场景风格之间建立更细的人工判定。
- 单色雕塑城堡自动分数为 0，标记裁切、碎片、主体小和复杂背景，但总览中的实际雕塑主体清晰完整。原因可能是后续单色/透明背景分析把大面积浅色城堡与透明边缘混淆，必须通过更多建筑样本确认后再改质量门禁。
- 25 张中 19 张出现 `palette_material_is_fragmented`，远高于肉眼看到的真实失败比例。该警告更像现有确定性色板分区对细节边界过敏，下一阶段需要统计连通域面积分布并与联络表交叉验证，而不是直接收紧 Image2 提示词。
- 第二批显示茶壶、吉他、牌楼的浮雕均形成明确实心底板，浮雕路线在有强正面轮廓的器物/建筑上继续稳定；盆景浮雕也把分枝收束到板面，适合后续 3D 深度约束。
- 工业机械臂的浮雕原始输出没有底板，仍是独立立体机械臂，而且保留/生成了 `CEYLAN ROBOT` 与 `KUKA` 文字。其持久化提供方提示词同时包含“浮雕必须在实心底板”和后文“无底座非人主体必须保持无底座”两条冲突约束；这是明确的提示词优先级缺陷，不是随机审美差异。
- 茶壶微缩场景同样没有共享场景底座，而是独立茶壶；提示词先要求“所有物体连接到共享底座”，后文又禁止给无底座的非人主体增加平台。浮雕机械臂与微缩茶壶共同证明样式特定支撑规则被通用无底座规则覆盖。
- 由于 v1 每个候选冻结并校验提供方提示词哈希，当前立刻修正生产提示词会让已完成 relief/diorama 候选与后续 dry-run 不兼容。正确顺序是先完成 v1 基线，再修改为“浮雕底板/微缩共享底座是样式级例外”，用独立 v2 输出目录对同一失败样本复测。
- 盆景的低多边形候选在提供方连接阶段失败一次，没有输出；错误状态已阻止自动重复付费调用。该项属于服务可用性证据，不计入风格质量失败率，后续汇总单列。
- 累计 49 张中 `palette_material_is_fragmented` 出现 38 次，但第一页肉眼可用率明显高于警告比例，进一步支持该标志应作为复核提示而非硬失败。真正需要优先修的是样式/通用构图规则冲突。
- 第三批单色雕塑中，章鱼把触手与岩体合成连续雕塑，相机、电钻和双鞋都保留主要部件/数量，说明单色材质并未系统性删减非人主体；哈士奇唯一失败是服务返回均匀图，不能归因于雕塑提示词质量。
- 章鱼五风格都保留八足主体与岩穴关系；哈士奇的手办、低多边形、浮雕和微缩场景稳定保留耳、口鼻、毛色分区和坐姿；运动鞋五种输出保留双鞋数量，数量约束继续有效。
- 相机和电钻的浮雕都形成明确矩形底板，器物浮雕的总体成功率仍高；但相机浮雕几乎贴满画面，自动门禁标记 `subject_nearly_fills_frame`，后续提示词可为底板外缘预留固定安全边距。
- 复古相机的微缩场景仍是无共享底座的孤立相机，成为继茶壶之后第二个“孤立器物 diorama 被通用无底座规则覆盖”的实例。无绳电钻的微缩场景则凭空增加岩石状底座，说明模型在冲突约束下会在“无底座”和“任意装饰底座”之间不稳定摆动。
- 电钻的手办、低多边形、浮雕和微缩场景均保留/生成了品牌式字样；工业机械臂也出现品牌/公司文字。尽管持久化提示词明确要求移除 text/logos，机械产品的高对比标签仍重复泄漏，v2 应把“所有文字转为无字宽槽/色块，不生成可读字符”提升到风格配置附近，而不是只放在长通用合同中。
- 累计 73 张中耗材碎片警告达到 58 次（79.5%），而硬门禁通过 97.3%。该指标与肉眼可用性相关性继续偏低；最终报告应把它列为确定性色板流水线待校准项，不把它描述为 Image2 风格失败率。
- 第四批低多边形是目前最能说明风格可用性的对照：石狮把鳞片/鬃毛压成大切面但保留张口与底座；爱因斯坦保留发型、胡须和脸部朝向；卓别林保留礼帽、手杖、全身姿态；自行车保留两轮、三角车架、把手与座椅；伞下猫保留一个伞和一只猫及遮挡关系。
- 低多边形累计唯一硬门禁失败仍是“双猫被判分离结构”，而肉眼结果正确保留两个合法主体；另一个缺口是盆景连接失败、没有生成图。由此看低多边形提示词本身没有出现系统性删件或退化，优先级低于支撑规则冲突。
- 人物非写实路线没有回到恐怖谷写实：爱因斯坦手办是温和夸张的收藏胸像，低多边形是清晰切面头像，浮雕是圆形背板胸像；卓别林手办/低多边形保留标志性礼帽、胡须、手杖与站姿。
- 卓别林浮雕没有生成底板，而是立体全身像；这说明 `_portrait_display_base_direction()` 的“人物必须低底座”也会覆盖 relief 的实心板要求。样式特定支撑规则必须同时优先于通用非人无底座规则和通用人物展示底座规则。
- 爱因斯坦/卓别林的微缩场景都退化为单人物展示底座，没有形成可区分的场景层级；结合茶壶、相机的无场景输出，`diorama` 对孤立主体系统性趋近普通手办。v2 需要明确：来源无场景时用最小语义地台/少量来源可见环境线索，但禁止凭空装饰；若用户只要孤立主体，推荐层面不应首推微缩场景。
- 自行车浮雕和雨伞浮雕形成底板，自行车微缩场景形成连续铺装底座，雨伞微缩场景保留一伞一猫并以共同地面连接；薄杆/轮辐/遮挡组合在这两例中没有掉件。
- 累计 98 张的耗材碎片警告为 78 次（79.6%），比例几乎不随类别变化，进一步表明它主要反映当前量化/连通域算法，而不是某一种 Image2 风格质量。
- 文本压力用例验证了“没有原图也能覆盖结构缺口”：早餐托盘保留一个托盘、羊角包、杯子、三颗浆果和餐巾；咖啡角保留吧台、两张凳、吊灯和盆栽；露营地保留帐篷、树、石圈和背包；水母灯把透明材质稳定改成实心分层；棋组能表达深色主体且轮廓可读。
- 文本生成的浮雕约束弱于图生图：早餐托盘可解释为圆盘浅浮雕，但露营地和水母灯的 relief 仍是独立三维摆件/场景，没有统一背板。说明 v2 的“浮雕底板是样式级硬例外”必须同时接入 `_style_preview_prompt` 与 `_text_image_prompt`，不能只修图生图合同。
- 文本场景中五种风格的内容清单整体稳定，但场景类输入会让手办、低多边形、浮雕和微缩场景的构图趋同；风格差异主要体现在曲面、切面、底板/地台和配色。最终推荐应强调：`diorama` 面向多元素场景，`relief` 面向单视图压扁表达，孤立主体不建议默认选择微缩场景。
- 水母灯五种输出均把透明伞盖/触手转换为不透明实体并保留 8 条粗触手的总体语言，证明“透明改实心层”方向有效；但细小吸盘装饰仍可能产生碎片，后续硬例验证要结合最小可打印细节而非仅看 Alpha。
- 深色棋组没有依赖黑色反光保持识别，五种输出以结构色和轮廓区分棋子；重复部件数量需在最终资源清单中人工标记，因为当前图片质量分不会验证“恰好六枚”。
- 批次结束时一次旧候选 `state.json` 被 Windows 短暂占用，导致最终 `write_summary` 读失败；候选状态全部完整，`--report-only` 恢复成功。既有写路径已有锁退避，读路径此前没有，这是可靠性缺口而非提供方失败。
- 家具阅读角五种输出都保留座椅、边桌、落地灯和书籍组；微缩场景形成连续低底座且空间层级清晰，符合 `diorama` 预期。手办/低多边形也能表达多物场景，但与微缩场景相比底面语义更弱。
- 家具阅读角的 relief 仍是独立三维家具组合，没有背板；加上露营地、水母灯和卓别林，浮雕失去底板已经跨图生图/文生图、人物/器物/场景重复出现，达到生产修正证据门槛。
- v1 基线的自动门禁会把所有 relief/diorama 判通过，但人工总览能发现样式语义失败，说明当前质量器只衡量一般图像/连接/色板条件，没有验证“浮雕必须背板”“微缩必须共享地台”“不得生成可读文字”。v2 需要新增可审计的人工判定字段或轻量确定性启发，而不能只依赖总分。
- 130 次调用中两个失败分别是一次连接中断和一次均匀输出；完成率 98.46%。这两项是提供方/传输失败，不应混入风格语义成功率，也不应在没有明确用户指示时重试。
- 128 个完成结果覆盖 26 个用例 × 5 种风格（仅两个单项缺失），包括 20 个许可审计网络输入和 6 个可控文本压力输入。覆盖范围已经足以先修高置信度规则冲突，无需继续为数量下载相似素材。
- v2 将支撑语义提升到风格配置附近后，机械臂、茶壶、相机、电钻、卓别林和 6 组文本场景的 11 个浮雕全部出现明确实心背板；这证明 v1 的无板问题主要来自提示词优先级冲突，而不是 Image2 无法理解浮雕。
- v2 微缩场景的工业机械臂、茶壶、相机、电钻都获得一个连续低地台，早餐托盘、咖啡角、露营地、棋组和阅读角也保持共享支撑面。孤立人物仍只能用最小地台表达，视觉上与普通手办接近，因此推荐层面继续不应把 `diorama` 首推给没有环境关系的单人物。
- 高优先级非写实文字清理对机械输入有效：工业机械臂控制柜、复古相机面板和电钻机身保留结构色块/凹槽，但 v2 总览中不再出现 v1 的 `CEYLAN ROBOT`、`KUKA` 或品牌式拼写。
- v2 自动门禁的 4 个阻塞都集中在单色雕塑的多元素文本场景（早餐托盘、水母灯、棋组、阅读角），但人工图中主体清晰且通过托盘、基座、棋盘或地面接触。单色分区启发式仍会把同材质物件间的阴影缝识别成分离组件，不能据此否定样式质量。
- `palette_material_is_fragmented` 在 v2 仍出现 43/55（78.2%），与 v1 约 79% 基本一致，而人工可用性显著提升；该警告稳定地反映当前色板/连通域算法偏敏感，不是新提示词回归。
- 文本输入在总览第一列显示 `PENDING` 会让已完成结果看似未生成；改成 `TEXT INPUT` 后能明确区分“没有来源图”和“候选尚未完成”，不改变任何候选或付费状态。
- 许可来源审计显示现有 20 张图已经覆盖 20 个高差异单图用例；食物、家具和室内外场景等图片集缺口已经由 6 个可控文本用例补足。此时继续抓取相似网络图不如把时间用于资源可追溯性、回归和报告，04:30 检查点因此没有扩展调用量。
- 两个批次合并后的资源总数是 185 个候选、183 个完成图片和 2 个失败状态；每种非写实风格各有 37 个候选。完整项的 raw、model-reference、contact-sheet 和 overview-sheet 路径均为 0 缺失，可支持飞书清单、人工抽查和后续复现。
- 飞书在线表格不适合逐张手工粘贴 185 个本地路径；精简指标与完整资源明细分为两个 TSV，可以把首页保持简洁，同时保留按风格/类别/状态筛选的明细导入能力。
- v1 在生产提示词修正后必然出现 130 个冻结提示词哈希不一致；这正是冻结清单防止把旧输出冒充新提示词结果的安全行为。v2 的 55 个候选全部可复用且哈希一致，最终 dry-run 两边都计划 0 次付费调用。
- 两页同屏对照比独立总览更能解释优化因果：机械臂/卓别林/阅读角/露营地/水母灯的 relief 从自由站立结果变为背板；茶壶/相机/电钻的 diorama 获得连续地台；电钻的品牌式文字消失。它们应作为飞书摘要中的主要视觉证据，而不是逐张展示 183 张完成图。
- 全量 504 条 AI 测试通过说明提示词优先级、评测器、资源导出和既有 Sidecar/模型流程没有出现回归；测试日志中的“paid Tripo”是合同测试固定文案，所有相关请求都停留在 Mock/loopback 环境，真实 Tripo 调用仍为 0。
- 新增的飞书 Sheets 专用接口能解析 Wiki、查询工作表并读取范围，说明之前 `docx` 权限不足不是读取 Sheet 的最终阻碍；但真实 PUT 返回 `91403 Forbidden`，当前应用权限仍不足以修改该表格。
- 目标 `Sheet1` 的 A1:H30 全为空，因此浏览器兜底不会覆盖用户现有单元格；最终矩阵只有 240 个标量单元格，内容已在 API dry-run 验证，确认后只需一次粘贴并读回检查。
- 十小时窗口结束时继续按小时检查已经没有收益：Image2 两批状态冻结、计划调用为 0、全量回归完成，唯一剩余动作需要用户确认。删除心跳既满足截止时间，也避免无意义重复通知；确认后可在普通交互中恢复飞书写入。
- 飞书在线表格在浏览器中忽略了程序化批量剪贴板粘贴，但干净的新标签页可以稳定地通过“地址框定位 → 简洁公式栏输入 → Enter 提交”逐格写入；每次提交后等待公式栏回到非编辑态可以避免多个值串入同一编辑缓冲。
- `Sheet1!A1:H30` 最终读回 revision 211。飞书把 `2026-09-02` 存为日期序列、把 URL/部分文件名存为富文本，并会把 `11/11` 自动识别为日期；将后两处比值单元格改为纯文本后，按显示语义归一的 240 单元格比对为 0 处差异。

# 2026-09-02 阶段 90：发布审计发现

- 模型功能版本已冻结为 `320394e429`。该提交相对既有集成来源 `5081891766` 的增量才是本次应移植范围；后续规划日志更新不改变这一本次产品来源 SHA。
- 官方主线预合并的 4 个冲突全部是双方同时演进的仓库/CI 配置，不涉及模型生成业务实现。`.gitattributes` 需要同时保留内部发布服务器脚本 LF 与官方批处理 CRLF；3 个工作流需要把官方构建脚本/编译器演进和内部 AI 商业包守卫合并，而不能直接选择 ours/theirs。

- 当前仓库远端为 `origin=https://github.com/arsenaltj/OrcaSlicer.git`、`upstream=https://github.com/OrcaSlicer/OrcaSlicer.git`；本地可见分支没有 `main`，只有已失去 origin 跟踪的 `master` 以及模型、切片和集成分支，因此必须先 fetch 再解释“主分支”。
- 用户既有发布偏好明确要求官方同源集成与发布只在 `codex/orca-integration-v2` 进行，功能分支只通过完整验收 SHA 交付；当前请求没有授权改写历史或反向合并集成线，应保留该边界。
- 仓库根没有 `.openai/hosting.json`，所以不是 Sites 托管项目；`website/` 是现有仓库目录，实际网站发布机制需要从集成工作树和发布脚本中继续审计，不能假定为 OpenAI Sites。
- 当前未跟踪项混有正式设计/测试脚本与数千个 `tmp/`、`.tmp/`、`validation/` 产物。内部版本提交应精确列举正式文件，不能使用无审计的 `git add -A`。
- `origin/HEAD` 指向 `origin/codex/orca-integration-v2`，这就是 fork 的实际默认/发布主线；`origin` 不存在 `main`。用户所说“主分支同步最新代码”应解释为让这条集成主线同时包含最新 `upstream/main` 和当前模型版本，而不是新建一个含义重复的 `main`。
- 当前 `origin/codex/orca-integration-v2=bbdfc62618` 已包含便携内部发布、受限上传和物理耗材槽修复；本地集成工作树只需安全快进 1 个提交后再同步官方主线。
- `upstream/main=b6ef6cf1be` 相对集成共同基线 `db29f570bd` 新增 81 个提交；集成侧有 29 个独有提交。现有集成线已经把模型分支旧 SHA `5081891766` 的能力移植成独立提交 `3c0cfe33be`，所以新版本应以 `5081891766..新验收 SHA` 的增量移植，直接 merge 功能分支会带入错误的长历史和大量旧基线冲突。
- 内部发布工具链已在 `release/` 与 `scripts/package_internal_fast.ps1` 中实现。网站源码是本机配置引用的独立工作树，集成仓库本身没有 `website/`；网站发布应由工具按配置执行，并在部署前检查该独立工作树是否干净或只含本次版本元数据。
- `release/build_internal.ps1` 会锁定分支、完整 source SHA、干净工作树、外置 defaults 与构建目录归属；随后调用权威快速打包器、集成守卫、原生定向测试，并验证 manifest、安装包哈希、7-Zip 与签名状态。该脚本应作为内部版本唯一构建入口。
- `release/upload_installer.ps1` 支持受限发布者分块续传并在服务器侧重复校验文件名、大小、SHA、source、revision 与 `MZ`；`verify_public_release.ps1` 会验证主页元数据、匿名下载、16 字节 Range、总大小、checksum header 和 `/healthz`，足以覆盖用户要求的“发布到网站”。
- 集成工作树当前没有本机 `release/config.local.ps1`。真实目标/凭据不可写进公共仓库或规划文件；只能在本机既有配置中定位并由发布脚本消费，找不到时应把网站发布标记为外部配置阻塞，而不是猜测服务器。
- 候选提交中的正式未跟踪内容体积可控：设计/架构 Markdown、发布报告、1 张质量对照 PNG 和 13 个 Python 生产/测试脚本；数千个 `.tmp/tmp/validation` 文件都是本地工具、构建缓存或验收图，不属于源码发布。
- 公开网站仓库的 `main` 落后于服务器正在运行的 28 项未提交网站功能；本次若从公开仓库直接部署会回退工作流和文档能力。发布因此以服务器现行源码为基线，只替换 `app/public.py`、`app/templates/home.html` 和 `tests/test_public.py` 三个版本文件。
- 新安装器已在网站下载目录原子落盘，服务端大小和 SHA-256 与 manifest 完全一致；站点部署使用隔离候选、两轮 37 项测试和维护脚本，没有手工切换 `current` 软链接。
- 公网验证读回了完整 source commit、CST 构建时间、文件名、四条更新说明、匿名下载按钮、MZ Range、总大小和 checksum header；这比仅检查首页文本更能证明网站与二进制是同一个版本。
- 390px 页面 `scrollWidth=375`、桌面 1456px 页面 `scrollWidth=1441`，均未超过视口；历史 Task ID 与三联对照等长文案在发布卡片中正常换行。

## 2026-09-02 阶段 90 最终集成发现

- 既定权威流程不是由功能任务直接修改并推送发布主线，而是先固定并推送功能 SHA，再通知现有“推进 OrcaSlicer AI 路线图”任务，由该任务独占 `codex/orca-integration-v2` 的审计、集成与远端提交。发现偏离后，未推送直接集成候选，改回该流程并由 v2 任务独立复核。
- GitHub 对中间集成候选 `d337b342979e0ae9aba8aedf31b5b5a980d66dd8` 暴露了一个既有 Linux Ninja Multi-Config 缺陷：`BuildInfo.cpp` 已由 `configure_file` 在配置时生成，却又被标记为 `GENERATED`，CMake 3.31 因而产生不存在的跨配置顺序依赖。最终提交 `7e9567897b4ceaa40c1e18168d4ad151ad8be18e` 删除错误属性并增加两类回归守卫。
- 最终功能 SHA `16f8ac0fb048570edb5fe787bdfdfcde4c9fa4b7` 还修复了 Windows 长短路径别名比较、测试中显式兼容网关和 `samefile()` 路径判定；这三处是 GitHub 环境与本机路径表现不同所暴露的可移植性问题，不改变产品功能契约。
- 最终网站继续以服务器现行源码为基线，只替换发布元数据三文件；因此没有覆盖站点上 28 项未提交功能。正式部署前保留独立备份，隔离副本与正式源码测试均通过。
- 远端最终引用已独立核验：`codex/model-generation=16f8ac0fb048570edb5fe787bdfdfcde4c9fa4b7`，`codex/orca-integration-v2=7e9567897b4ceaa40c1e18168d4ad151ad8be18e`，新标签 peeled SHA 与集成线一致，两个旧内部标签没有移动。
