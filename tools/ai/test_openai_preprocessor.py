...
class StylePreviewPromptTests(unittest.TestCase):
    ...
    def test_short_user_instruction_is_wrapped_with_preview_constraints(self):
        ...
        self.assertIn("Portrait base policy", prompt)
        self.assertIn("keep the generated portrait base-free", prompt)
        self.assertIn("every generated portrait must be free of a separate display base", prompt)
        self.assertIn("Do not preserve, recreate, or invent a source-visible base", prompt)
        self.assertIn("Orca may add a selectable round, oval, or rectangular display base", prompt)
        self.assertNotIn("always add exactly one low, simple, integrated display base", prompt)
        ...
        self.assertIn("end the lower torso with a clean printable boundary", prompt)
    ...
    def test_custom_style_is_wrapped_without_overriding_print_constraints(self):
        ...
        self.assertIn("allowed printable palette", prompt)
        self.assertIn("keep the generated portrait base-free", prompt)
        self.assertIn("non-human standing, seated, crouched", prompt)
...
class TextImagePromptTests(unittest.TestCase):
    ...
    def test_printable_text_image_prompt_uses_large_regions_and_rejects_dithering(self):
        ...
        self.assertIn("keep the generated portrait base-free", prompt)
        self.assertIn("finish the visible lower torso or clothing with a clean printable boundary", prompt)
