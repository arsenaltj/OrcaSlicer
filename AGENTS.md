...
- Every PR includes the versioned architecture impact visualization produced by `PR architecture impact`: a Mermaid module diagram plus downloadable Archify HTML, complete dependency details and JSON, all generated from the same code-review-graph analysis. For local preview use `./dev.ps1 Review` (includes uncommitted changes) or `./dev.ps1 Review -Committed`; see [architecture review](Docs/coordination/architecture-review.md). Keep direct changes, inferred impact and uncovered files distinct. An unavailable graph is a reporting gap, never evidence that a change is safe; this visualization does not replace required tests or review and does not trigger an application build.
- When business phases, states, module ownership, source anchors or product capabilities change, update the affected entries of `Docs/architecture/review-map.json`. Preserve unimplemented goals as `未做`; do not infer completion from file presence. Run `python scripts/architecture_review.py --readme update` to synchronize the generated README overview (also done by worktree `dev.ps1 Review`) and include resulting changes in that PR. Ordinary internal edits need no prose rewrite if the mapping remains accurate. CI checks the README and generates each PR's diff report; do not install global skills, create planning files or run full application builds for this workflow. Never hand-edit between README architecture markers.
