...
option(ORCA_ENABLE_MOBILESAM_NATIVE "Enable optional local MobileSAM CPU boundary refinement" ${_orca_semantic_native_default})
...
function(orca_configure_semantic_recognizers target)
    ...
    if(ORCA_ENABLE_MOBILESAM_NATIVE)
        if(NOT _semantic_x64)
            message(FATAL_ERROR "ORCA_ENABLE_MOBILESAM_NATIVE currently supports Windows x64 only")
        endif()
        target_compile_definitions(${target} PRIVATE ORCA_ENABLE_MOBILESAM_NATIVE=1)
        target_include_directories(${target} PRIVATE "${_semantic_source_dir}/src/slic3r/AI/ModelGeneration/SemanticColoring/vendor/onnxruntime-1.22.1")
        set_property(TARGET ${target} APPEND PROPERTY LINK_LIBRARIES bcrypt)
        get_target_property(_boundary_target_type ${target} TYPE)
        if(_boundary_target_type STREQUAL "STATIC_LIBRARY")
...
function(orca_stage_semantic_runtime target)
    ...
    foreach(_semantic_file IN LISTS _orca_semantic_runtime_files)
        get_filename_component(_runtime_subdir "${_semantic_file}" DIRECTORY)
        install(FILES "${ORCA_SEMANTIC_RUNTIME_DIR}/${_semantic_file}" DESTINATION "ai/portrait_semantics/${_runtime_subdir}")
    endforeach()
endfunction()
