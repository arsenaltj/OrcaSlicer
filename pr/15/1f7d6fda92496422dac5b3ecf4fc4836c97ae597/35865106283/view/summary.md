...
版本：`decf64fab128` → `1f7d6fda9249`；模式：PR 提交。

直接修改 **251** 个文件，潜在关联 **50** 个文件；其中 **108** 个改动文件未做代码关系解析。

黄色＝直接修改，蓝色＝潜在关联；箭头由使用方指向依赖方。

```mermaid
flowchart LR
  desktop["桌面与运行时 · 修改 22 / 关联 0"]
  generation["模型生成 · 修改 75 / 关联 27"]
  slicing["智能切片 · 修改 0 / 关联 2"]
  contracts["共享契约 · 修改 5 / 关联 1"]
  adapter["Orca 适配 · 修改 18 / 关联 2"]
  core["Orca 核心 · 修改 19 / 关联 0"]
  tests["测试 · 修改 70 / 关联 18"]
  engineering["开发与协作 · 修改 36 / 关联 0"]
  assets["资源与依赖 · 修改 1 / 关联 0"]
  unknown["待归属 · 修改 5 / 关联 0"]
  adapter -->|依赖| contracts
  adapter -->|依赖| desktop
  adapter -->|依赖| generation
  adapter -->|依赖| slicing
  desktop -->|新增| adapter
  desktop -->|新增| contracts
  ...
  class desktop changed
  class generation changed
  class slicing potential
  class contracts changed
  class adapter changed
  class core changed
  class tests changed
  class engineering changed
  class assets changed
