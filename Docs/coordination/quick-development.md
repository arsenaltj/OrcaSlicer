...
入口从已有 CMake cache 获取 Python/CMake，默认 `build-validation` 的 Release 配置；其他目录用 `-BuildDir <目录>`，必要时用 `-Configure`，并行度用 `-Jobs 2`。另一台电脑必须使用自己的源码、依赖和构建目录，不复制 CMakeCache、对象文件、PDB 或运行记录。首次配置参考[构建存储说明](local-build-storage.md)，其中机器路径是示例，不是跨电脑共享配置。

`Test` 必须指定文件名或 glob，复用现有离线测试器的凭据清理与 Python 外网防护；不编译、不复制运行环境、不启动 GUI。测试失败或未匹配均失败。此防护不是 OS 沙箱，测试仍须实际隔离提供商。
...
`Run` 构建 `OrcaSlicer_app_gui` 及依赖，通过 CMake install 准备完整运行文件，不生成安装包。复用 `.tmp/dev/run` 和独立配置 `.tmp/dev/data`；更新前正常关闭该试用实例，保留其他实例和用户工程。构建期间不要改源码。`Sidecar` 只刷新安装清单中的 Python 模块，原生源码、资源、配置或二进制变化时要求先运行 `Run`，不能用旧 EXE 验收新代码。
