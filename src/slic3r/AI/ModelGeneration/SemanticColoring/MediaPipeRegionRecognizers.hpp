...
struct RegionRecognizers {
    ...
    std::unique_ptr<IPoseRegionRecognizer> pose;
...
using PoseRecognizerFactory = std::function<std::unique_ptr<IPoseRegionRecognizer>(const std::filesystem::path&)>;
...
bool register_pose_recognizer_factory(const std::string& id, PoseRecognizerFactory);
...
RegionRecognizers create_region_recognizers(const std::string& body_provider,
                                           ...
                                           const std::string& pose_provider,
